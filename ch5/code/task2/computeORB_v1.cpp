...
//                    {
//                        for (int i = u-8; i < u+7; ++i) {
//                            for (int j = v-8; j < v+7; ++j) {
//                                m10 += i * image.at<uchar>(i,j);
//                                m01 += j * image.at<uchar>(i,j);
//                            }
//                        }
//                    }
//                    //atan会返回弧度制的旋转角,但 OpenCV 中使用角度制,需要进行弧度转换
//                    kp.angle = (double)atan(m01/m10)*180/pi;
//                });
  return;
}
  ...
  for (auto &kp : keypoints) {
    ...
    for (int i = 0; i < 256; i++) {
        ...
        if(u-8>=0 && v-8>=0 && u+7 <= image.cols && v+7 <= image.rows)
        {
            ...
            d[i] = image.at<uchar>(u_p_,v_p_) > image.at<uchar>(u_q_,v_q_) ? false : true;
        }else{
            d.clear();
        }
        // END YOUR CODE HERE
...
void bfMatch(const vector<DescType> &desc1, const vector<DescType> &desc2,
  ...
  for (int i=0; i<desc1.size(); i++)
  {
      ...
      for (int j = 0; j < desc2.size(); ++j) {
          ...
          //计算两个描述子的汉明距离
          for(int k=0; k<256; k++){
              hanming_distance += desc1[i][k]^desc2[j][k];//异或，相同取0,不同取1
          }
